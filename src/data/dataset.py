...
import data.glioma_dataprep as dataprep_glioma
...
def parse(ds_name, ntasks, balanced=True, num_class=10, noisy_center=5):
    ...
    elif ds_name == GliomaDataset.argname:  # Supplemental
        return GliomaDataset(task_count=ntasks, balanced=balanced, num_class=num_class)
    else:
...
class GliomaDataset(CustomDataset):
    name = 'Glioma'
    argname = 'glioma'
    test_results_dir = 'glioma'
    train_exp_results_dir = 'glioma'
    def_task_count, task_count = 4, 4
    classes_per_task = OrderedDict()
    input_size = (256, 256)

    def __init__(self, crop=False, create=True, task_count=4, dataset_root=None, overwrite=False, balanced=True, num_class=1):
        config = utils.get_parsed_config()

        self.dataset_root = dataset_root if dataset_root else os.path.join(
            utils.read_from_config(config, 'ds_root_path'), 'glioma', 'glioma_preprocessed')
        print("Dataset root = {}".format(self.dataset_root))
        self.crop = crop
        self.task_count = task_count
        ...
        if create:
            # dataprep_glioma.download_dset(os.path.dirname(self.dataset_root))
            dataprep_glioma.prepare_dataset(self, self.dataset_root, task_count=self.task_count, survey_order=True,
                                          overwrite=overwrite, num_class=num_class)
        # Dataset with bare 64x64, no 56x56 crop
        if not crop:
            self.dataset_root = os.path.join(self.dataset_root, 'no_crop')
